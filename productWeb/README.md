...
1. Clonar el repositorio:
   ...
   ```
2. Instalar dependencias e iniciar la API de usuarios:
   ```bash
   cd backend
   npm install
   npm start
   ```
3. En otra terminal, hacer lo mismo con la API de productos:
   ```bash
   cd backendProducts
   npm install
   npm start
   ```
4. (Opcional) Cargar los datos de prueba en la base de datos `mongobootcamp`:
   ```bash
...
Para ver la documentación, se puede abrir el archivo `api/openAPI-1.1.6.yaml` de cada API en [Swagger Editor](https://editor.swagger.io/).
